chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'updateTheme') {
    const theme = message.theme;

    // Remove existing themes
    document.body.classList.remove('dark-theme', 'osu-theme');
    
    // Remove the injected CSS if "no-theme" is selected
    const existingStyle = document.getElementById('customTheme');
    if (existingStyle) {
      existingStyle.remove();
    }
    
    if (theme !== 'no-theme') {
      // Add the new theme class
      document.body.classList.add(theme);
      
      // Inject CSS to enable the selected theme
      const style = document.createElement('style');
      style.id = 'customTheme';
      if (theme === 'dark-theme') {
        style.textContent = `

html, body {
  background-color: #b12124;
  background: #b12124; /* ifsta red */
  /* background:#FF7300; /* osu orange */
  /* background: #000;  /* dark mode */
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #b12124;
}

h1 {
    color: #b12124;
}


h2 {
    color: #b12124;
}

.dialog h3 {
    color: #b12124;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #b12124;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #b12124;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}

#nav .active a {
    color: #b12124;
}

#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
}


#nav .inactive a {
    color: #b12124;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #b12124;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #b12124;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #b12124;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #b12124 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #b12124 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #b12124;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #b12124 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #b12124;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: black;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #fff;
}


.action-dropdown ul li.active > a {
    background-color: #b12124;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #b12124;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #b12124;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #000;
}

.faded {
  color: #000;
}

.Icon {
  color: #b12124
}

a, .link {
  color: #b12124
}


.blue.button, .blue.button:visited {
    background-color: #b12124;
    
}

.blue.button:hover {
    background-color: #000;
    color: #b12124
}

.green.button, .green.button:visited {
    background-color: #b12124;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #b12124 inset;
    color: #b12124;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #b12124;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}



.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #b12124;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #b12124;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #b12124;
    background: #000;
    color: #b12124;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #b12124;
}

.link.button:hover, .link.button:active {
    background-color: #b12124;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #b12124;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #b12124;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #b12124;
    background-color: #000;
}

.dialog a {
    color: #b12124;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#b12124;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #b12124;
}

.filedrop .dropzone a {
    color: #b12124;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #b12124;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #b12124;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #b12124 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #b12124;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #b12124 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #b12124;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #b12124;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #b12124;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}
.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    border-left: 5px solid #7d7d7d;
    color: #333;
}
        `;
      } else if (theme === 'osu-theme') {
        style.textContent = `

html, body {
  background-color: #FF7300;
  background: #FF7300; 
  /* background:#b12124; /* ifsta red */
  /* background: #000;  /* dark mode */
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #FF7300;
}

h1 {
    color: #FF7300;
}


h2 {
    color: #FF7300;
}

.dialog h3 {
    color: #FF7300;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #FF7300;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #FF7300;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}


#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #FF7300;
}

#nav .active a {
    color: #FF7300;
}

#nav .inactive a {
    color: #FF7300;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #FF7300;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #FF7300;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #FF7300;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #FF7300;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #FF7300 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #FF7300 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #FF7300;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #FF7300 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #FF7300;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: black;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #fff;
}


.action-dropdown ul li.active > a {
    background-color: #FF7300;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #FF7300;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #FF7300;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #000;
}

.faded {
  color: #000;
}

.Icon {
  color: #FF7300
}

a, .link {
  color: #FF7300
}


.blue.button, .blue.button:visited {
    background-color: #FF7300;
    
}

.blue.button:hover {
    background-color: #000;
    color: #FF7300
}

.green.button, .green.button:visited {
    background-color: #FF7300;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #FF7300 inset;
    color: #FF7300;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #FF7300;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}
FF7300


.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #FF7300;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #FF7300;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #FF7300;
    background: #000;
    color: #FF7300;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #FF7300;
}

.link.button:hover, .link.button:active {
    background-color: #FF7300;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #FF7300;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #FF7300;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #FF7300;
    background-color: #000;
}

.dialog a {
    color: #FF7300;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#FF7300;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #FF7300;
}

.filedrop .dropzone a {
    color: #FF7300;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #FF7300;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #FF7300;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #FF7300 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #FF7300;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #FF7300 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #FF7300;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #FF7300;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #FF7300;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}

.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    padding: 10px 20px;
    margin: 0 0 20px;
    border-left: 5px solid #7d7d7d;
    color: #333;
}
        `;

      } else if (theme === 'midnight-theme') {
        style.textContent = `

html, body {
  background-color: #000;
  background: #000; 
  /* background:#FF7300; /* osu orange */
  /* background: #b12124;  /* ifsta red*/
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #b12124;
}

h1 {
    color: #b12124;
}


h2 {
    color: #b12124;
}

.dialog h3 {
    color: #b12124;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #b12124;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #b12124;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}


#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
}


#nav .inactive a {
    color: #b12124;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #b12124;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #000;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #b12124;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #000 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #000 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #000;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #b12124 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #b12124;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: #fff;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #000;
}


.action-dropdown ul li.active > a {
    background-color: #b12124;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #000;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #b12124;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #b12124;
}

.faded {
  color: #000;
}

.Icon {
  color: #b12124
}

a, .link {
  color: #000
}


.blue.button, .blue.button:visited {
    background-color: #b12124;
    
}

.blue.button:hover {
    background-color: #000;
    color: #b12124
}

.green.button, .green.button:visited {
    background-color: #b12124;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #b12124 inset;
    color: #b12124;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #b12124;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}



.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #b12124;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #b12124;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #b12124;
    background: #000;
    color: #b12124;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #b12124;
}

.link.button:hover, .link.button:active {
    background-color: #b12124;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #b12124;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #b12124;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #b12124;
    background-color: #000;
}

.dialog a {
    color: #b12124;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#b12124;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #b12124;
}

.filedrop .dropzone a {
    color: #b12124;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #b12124;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #b12124;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #b12124 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #b12124;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #b12124 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #b12124;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #b777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #b12124;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #b12124;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}

.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    padding: 10px 20px;
    margin: 0 0 20px;
    border-left: 5px solid #7d7d7d;
    color: #333;
}
        `;

      }
      document.head.appendChild(style);
    }
  }
});

// Apply the theme based on stored setting when the page loads
chrome.storage.sync.get(['theme'], (result) => {
  const theme = result.theme || 'no-theme';
  if (theme !== 'no-theme') {
    document.body.classList.add(theme);

    const style = document.createElement('style');
    style.id = 'customTheme';
    if (theme === 'dark-theme') {
      style.textContent = `

html, body {
  background-color: #b12124;
  background: #b12124; /* ifsta red */
  /* background:#FF7300; /* osu orange */
  /* background: #000;  /* dark mode */
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #b12124;
}

h1 {
    color: #b12124;
}


h2 {
    color: #b12124;
}

.dialog h3 {
    color: #b12124;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #b12124;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #b12124;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}

#nav .active a {
    color: #b12124;
}

#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
}


#nav .inactive a {
    color: #b12124;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #b12124;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #b12124;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #b12124;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #b12124 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #b12124 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #b12124;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #b12124 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #b12124;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: black;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #fff;
}


.action-dropdown ul li.active > a {
    background-color: #b12124;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #b12124;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #b12124;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #000;
}

.faded {
  color: #000;
}

.Icon {
  color: #b12124
}

a, .link {
  color: #b12124
}


.blue.button, .blue.button:visited {
    background-color: #b12124;
    
}

.blue.button:hover {
    background-color: #000;
    color: #b12124
}

.green.button, .green.button:visited {
    background-color: #b12124;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #b12124 inset;
    color: #b12124;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #b12124;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}



.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #b12124;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #b12124;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #b12124;
    background: #000;
    color: #b12124;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #b12124;
}

.link.button:hover, .link.button:active {
    background-color: #b12124;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #b12124;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #b12124;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #b12124;
    background-color: #000;
}

.dialog a {
    color: #b12124;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#b12124;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #b12124;
}

.filedrop .dropzone a {
    color: #b12124;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #b12124;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #b12124;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #b12124 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #b12124;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #b12124 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #b12124;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #b12124;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #b12124;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}
.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    border-left: 5px solid #7d7d7d;
    color: #333;
}
      `;
    } else if (theme === 'osu-theme') {
      style.textContent = `

html, body {
  background-color: #FF7300;
  background: #FF7300; 
  /* background:#b12124; /* ifsta red */
  /* background: #000;  /* dark mode */
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #FF7300;
}

h1 {
    color: #FF7300;
}


h2 {
    color: #FF7300;
}

.dialog h3 {
    color: #FF7300;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #FF7300;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #FF7300;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}


#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #FF7300;
}

#nav .active a {
    color: #FF7300;
}

#nav .inactive a {
    color: #FF7300;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #FF7300;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #FF7300;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #FF7300;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #FF7300;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #FF7300 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #FF7300 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #FF7300;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #FF7300 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #FF7300;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: black;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #fff;
}


.action-dropdown ul li.active > a {
    background-color: #FF7300;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #FF7300;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #FF7300;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #000;
}

.faded {
  color: #000;
}

.Icon {
  color: #FF7300
}

a, .link {
  color: #FF7300
}


.blue.button, .blue.button:visited {
    background-color: #FF7300;
    
}

.blue.button:hover {
    background-color: #000;
    color: #FF7300
}

.green.button, .green.button:visited {
    background-color: #FF7300;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #FF7300 inset;
    color: #FF7300;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #FF7300;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}
FF7300


.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #FF7300;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #FF7300;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #FF7300;
    background: #000;
    color: #FF7300;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #FF7300;
}

.link.button:hover, .link.button:active {
    background-color: #FF7300;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #FF7300;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #FF7300;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #FF7300;
    background-color: #000;
}

.dialog a {
    color: #FF7300;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#FF7300;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #FF7300;
}

.filedrop .dropzone a {
    color: #FF7300;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #FF7300;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #FF7300;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #FF7300 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #FF7300;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #FF7300 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #FF7300;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #FF7300;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #FF7300;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}

.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    padding: 10px 20px;
    margin: 0 0 20px;
    border-left: 5px solid #7d7d7d;
    color: #333;
}
      `;

      } else if (theme === 'midnight-theme') {
        style.textContent = `

html, body {
  background-color: #000;
  background: #000; 
  /* background:#FF7300; /* osu orange */
  /* background: #b12124;  /* ifsta red*/
 
  
   margin 0px;
   font-size: 14px;
}

#header {
    height 100px;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    background-color: #7d7d7d;
}

#header p {
    background: #bbb;
    border: 1px solid #000;
}

div#header a{
  color: #b12124;
}

h1 {
    color: #b12124;
}


h2 {
    color: #b12124;
}

.dialog h3 {
    color: #b12124;
}

#msg_info, .info-banner {
    margin: 0;
    padding: 5px;
    margin-bottom: 10px;
    color: #000000;
    border: 1px solid #b12124;
    background-color: #7d7d7d;
}


.dialog {
    background: #bbbbbb;
    border: 2px solid #b12124;
}

.dialog a.close {
    color: #000;
}

input[type="reset"] {
  color: #000;
}

input[type="button"]{
    color: #000;
}

input[type="submit"]{
    color: #000;
}


#nav {
    border-top: 1px solid #000;
    border-left: 1px solid #000;
    border-right: 1px solid #000;
    margin: 0px;
}

#sub_nav > li > a:hover, #customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
}


#nav .inactive a {
    color: #b12124;
    display: block;
}

a:hover, .link:hover {
    border-bottom: 1px  #000;
    margin-bottom: 0;
    cursor: pointer;
}

#nav .inactive li a:hover {
    color: #000;
    background-color: #fbfbfb;
    background-color: rgba(0, 0, 0, 0.05);
}

#nav {
    background: #ddd;
    border-bottom: 1px solid #000000;
}

#nav li a.active, #nav li a:hover {
    color: #000;
}

#sub_nav {
    background: #bbbbbb;
    border-bottom: 1px solid #b12124;
}

#basic_search {
    background-color: #a5a5a5;
    border-bottom: 1px dotted #000;
}

h2 i {
    color: #000;
}


.jb-overflowmenu {
    position: relative;
    height: 35px;
    width: auto;
}

#customQ_nav {
    border: 1px solid #000000;
    border-bottom: 1px solid #000;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover > a {
    color: #b12124;
    text-decoration: none;
}

#customQ_nav .jb-overflowmenu-menu-primary li.item:hover {
    background-color: #a5a5a5;
}

.jb-overflowmenu-menu-primary div.customQ-dropdown {
    background: #bbb;
}

.customQ-dropdown ul li:not(.personalQ):hover {
    color: #fff;
    background-color: #000;
    cursor: pointer;
}

.customQ-dropdown ul.scroll-height {
    border-bottom: 1px solid #b12124;
}

.configureQ {
    display: block;
    float: left;
    color: #000;
    margin: 4px 0 0 4px;
    cursor: pointer;
}

.configureQ:hover {
    color: #fff;
}

.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #000 inset;
}

a.action-button, .button {
color: #000;
  box-shadow: 0 0 0 1px #000 inset;
}

.action-button i.icon-caret-down {
    border-left: 1px solid #000;
}


#container {
    background-color: #7d7d7d;
    border: 1px solid #000000;
  
      width: 1020px;
  margin-top: 10px;
}


#content {
    background-color: #7d7d7d;
    border: 1px solid #000000;
    border-top: 1px solid #000;
    border-bottom: 1px solid #000;
    margin: 0px
}


table.list tbody td {
    border: 1px solid #000;
}



i.help-tip {
    vertical-align: inherit;
    color: #000;
    opacity: 0.8;
    text-indent: initial;
}

i.help-tip:hover {
    color: #b12124 !important;
    cursor: pointer;
    opacity: 1;
}

.action-dropdown ul{
    background: #bbbbbb;
}

.action-dropdown ul li > a {
    color: #000;
}


.noclick-dropdown ul li > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


.customQ-dropdown li:not(.personalQ) > span {
    color: #b12124;
    font-weight: bold;
    margin-right: 0;
}


.action-button:hover {
    color: #fff;
    box-shadow: 0 0 0 2px #000 inset;
    background-color: #ddd;
    background-color: #000;
}


.action-dropdown ul li.active > a {
    background-color: #b12124;
    color: #fff;
}


.action-dropdown ul li > a:hover, .action-dropdown ul li.active > a:hover{
    background-color: #000;
    color: #FFF !important;
    cursor: pointer;
}


table {
    border-collapse: collapse;
    border-spacing: 0;
    width: -webkit-fill-available;
}

#nav li.active {
    background-color: #a5a5a5;
}

#nav li.active, #nav li.inactive:hover {
    border-color: #000;
}

table.list thead th a.desc {
    background: url(../images/desc.gif) 100% 50% no-repeat #b12124;
  color: #fff;
}



#ticketForm td em {
    color: #000;
}

a:hover, .link:hover {
    margin-bottom: 0;
    cursor: pointer;
    color: #b12124;
}

.faded {
  color: #000;
}

.Icon {
  color: #b12124
}

a, .link {
  color: #000
}


.blue.button, .blue.button:visited {
    background-color: #b12124;
    
}

.blue.button:hover {
    background-color: #000;
    color: #b12124
}

.green.button, .green.button:visited {
    background-color: #b12124;
}

.green.button:hover {
    background-color: #000;
    box-shadow: 0 0 0 2px #b12124 inset;
    color: #b12124;
}



ul.tabs.clean li.active {
    background-color: #e7e7e7;
}


ul.tabs li.active {
    color: #b12124;
    background-color: #bbb;
    text-align: center;
    border: 2px solid #000000;
    border-bottom: none;
}

ul.tabs li {
  background-color: #a0a0a0;
}

ul.tabs li a {
  color: #000;
}

ul.tabs {
  background: #7d7d7d
}


input[type=checkbox] {
    accent-color: #000;
}



#footer {
    clear: both;
    padding: 0;
    text-align: center;
    font-size: 0.8em;
}



#loading h1, #upgrading h4 {
    margin: 3px 0 0 0;
    padding: 0;
    color: #000;
}



.thread-event {
    background: #7d7d7d;
}


.type-icon {
    background-color: #b12124;
    border: 1px solid #000;
}


.thread-entry::before {
    border-top: 2px solid #000;
}

.thread-entry::after {
    border-bottom: 2px solid #0000;
}


.thread-entry .thread-body {
    border: 1px solid #000;
    border-bottom: 2px solid #000;
}


.tip_menu li a {
    color: #b12124;
}


.tip_menu li a:hover {
    color: #000000;
}

#msg_warning, .warning-banner {
    border: 1px solid #b12124;
    background: #000;
    color: #b12124;
}

tr:hover i.help-tip, tr i.help-tip.warning {
    color: #fff;
}

.link.button, .link.button:active {
    background-color: black;
    color: #b12124;
}

.link.button:hover, .link.button:active {
    background-color: #b12124;
    color: #000000;
}

table.list thead th a.asc {
    background: url(../images/asc.gif) 100% 50% no-repeat #000000;
    color: #fff;
}

.sticky.bar.opaque {
    background-color: #7d7d7d;
}

.sticky.bar.fixed {
    background-color: #7d7d7d;
}

@media screen and (min-width: 1040px) {
    .scroll-up {
        background-color: #444;
        background-color: #b12124;
        color: #ddd !important;
        color: rgb(0 0 0) !important;
    }
}

.jb-overflowmenu .jb-overflowmenu-container > a.jb-overflowmenu-menu-secondary-handle {
    color: #b12124;
}

.jb-overflowmenu-menu-secondary li.item:hover {
    color: #b12124;
    background-color: #000;
}

.dialog a {
    color: #b12124;
}

.form_table {
    margin-top: 3px;
    border-left: 0px;
    border-right: 0px;
}
.form_table td {
    border-bottom: 0px;
    padding: 4px;
}

::selection {
color:#fff;
background-color:#b12124;
}

.redactor-toolbar a, .redactor-air a {
    background: #7d7d7d;
}

.redactor-toolbar a:hover, .redactor-air a:hover {
    outline: none;
    color: #000;
    background: #b12124;
}

.filedrop .dropzone a {
    color: #b12124;
}

.filedrop .dropzone {
    color: #313131;
}

.ui-widget-header {
    border: 1px solid #000000;
    color: #000;
    font-weight: bold;
    background: #b12124;
}

.ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
    border: 1px solid #000000;
    background: #b12124;
    color: #000000;
}

time.faq {
    color: #000;
}

.redactor-focus.redactor-styles-on, .redactor-focus:focus.redactor-styles-on {
    border-color: #000000 !important;
}

#response_options > form {
    background: #bbb;
    border: 1px solid #000;
    border-top: 1px solid #000;
}

.ticket_info {
    background: #7d7d7d;
}

.tixTitle h3 {
    color: #000;
}

.has_bottom_border {
    border-bottom: none;
}

.thread-body {
    color: #000000;
    background-color: #bbbbbb;
}

.thread-body a:not(.button) {
    color: #b12124 !important;
}

.redactor-box.redactor-styles-on {
    background: #bbb;
    border: 1px solid #7d7d7d;
}
.redactor-styles {
    color: #000;
}
.redactor-statusbar {
    background: #7d7d7d;
}

.redactor-box.redactor-styles-on .redactor-toolbar.redactor-toolbar {
    background-color: #7d7d7d;
}

div.selected-signature {
    background-color: #a5a5a5;
}

#nav li.inactive:hover {
    background-color: #bbb;
}

#nav li.inactive > ul {
    background: #bbbbbb;
}

.customQ-dropdown li h4, .customQ-dropdown li.top-level {
    background-color: #bbb;
}

table tfoot td {
    background: #a5a5a5;
    padding: 1px;
}

table.list {
    background: #000;
    border-bottom: 1px solid #000;
}

.save.pending {
    background-color: #b12124;
    box-shadow: 0 0 0 2px #000000 inset;
}

.save.pending:hover {
    box-shadow: 0 0 0 2px #b12124 inset;
    background-color: #000000;
    color: white;
}

.tip_content {
    border: 1px solid #000;
    background: #7d7d7d;
}

.thread-entry.message .header {
    background: #ff0000;
}

.thread-entry.response .header {
    background: #b12124;
}

.light .button:hover, .white.button {
    background-color: #7d7d7d;
    border-color: #000;
}

.thread-entry.note .header {
    background: #b777777;
}

.label-bare {
    border: 1px solid #000000;
    color: #000000;
}

.muted-button {
    border: 1px solid #00000080;
    color: #000000;
    background-color: rgba(0, 0, 0, 0.1);
    background: linear-gradient(0, #00000080, #ffffff80);
}

.thread-preview-entry.note .header {
    background-color: #777777;
}

.thread-preview-entry.collapsed.response .header .thread-name span:first-child {
    color: #b12124;
}

.thread-preview-entry.collapsed.message .header .thread-name span:first-child {
    color: #ff0000;
}

.thread-preview-entry.message .header {
    background-color: #fff;
}

.message.bar.warning {
    border-bottom: 3px solid #b12124;
}

.message.bar {
    background-color: #bbbbbb;
}

table.list thead th {
    background-color: #a5a5a5;
}

.select2-container--default .select2-selection--single {
    background-color: #a5a5a5;
    border: 1px solid #000;
}

.select2-container--default .select2-selection--single .select2-selection__rendered {
    color: #000;
}

.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #000 transparent transparent transparent;
}

.select2-dropdown {
    background-color: #bbbbbb;
    border: 1px solid #000;
}

.select2-container--default .select2-results__option--highlighted[aria-selected] {
    background-color: #000000;
    color: white;
}

.select2-container--default .select2-results__option[aria-disabled=true] {
    color: #000;
}

.customQ-dropdown ul li, .customQ-dropdown ul li > a {
    color: #000;
    white-space: nowrap;
}

.thread-body blockquote {
    padding: 10px 20px;
    margin: 0 0 20px;
    border-left: 5px solid #7d7d7d;
    color: #333;
}
        `;

    }
    document.head.appendChild(style);
  }
});
